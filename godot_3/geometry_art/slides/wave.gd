...
var colors = PoolColorArray([Color(0,0,0)])
...
func _input(event):
	if event.is_action_pressed("Up")    : get_tree().change_scene_to(load("res://final.tscn"))
	if event.is_action_pressed("Down")  : get_tree().change_scene_to(load("res://slides/matrix.tscn"))
	if event.is_action_pressed("Next")  : frequency *= 1.1
	if event.is_action_pressed("Prev")  : frequency *= 0.9
...
func draw_wave_arc_poly(center, color):
	var nb_points  = 32
	var points_distance  = 20
	...
	for y in range (22):
		...
		value =  sin( frequency * 0.51 + time + float(y) )
		value += sin( frequency * 0.172 + (time + float(y)/10) * 1.121 ) * 4.5
		value += sin( frequency * 0.2221 + (time + float(y)/10) * 0.437 ) * 5
		value += sin( frequency * 0.31122 + (time + float(y)/10) * 4.269 ) * 2.5

		value += sin( (y+time) * frequency * 5.51 - time + float(y) )
		value += sin( (y+time) * frequency * 1.172 - (time + float(y)/10) * 1.121 ) * 4.5
		value += sin( (y+time) * frequency * 2.2221 - (time + float(y)/10) * 0.437 ) * 5
		value += sin( (y+time) * frequency * 3.31122 - (time + float(y)/10) * 4.269 ) * 2.5
				
		value *= amplitude
		value -= (amplitude * 24)
		prev_point.y += value
		for i in range( nb_points ):
			next_point =  Vector2(center.x + (i+1) * points_distance, center.y)
			value =  sin(  frequency * 0.51 + time + float(y+i+1) )
			value += sin(  frequency * 0.172 + (time + float(y+i+1)/10) * 1.121 ) * 4.5
			value += sin(  frequency * 0.2221 + (time + float(y+i+1)/10) * 0.437 ) * 5
			value += sin(  frequency * 0.31122 + (time + float(y+i+1)/10) * 4.269 ) * 2.5

			value += sin( (y+time) * frequency * 5.51 - time + float(y+i+1) )
			value += sin( (y+time) * frequency * 1.172 - (time + float(y+i+1)/10) * 1.121 ) * 4.5
			value += sin( (y+time) * frequency * 2.2221 - (time + float(y+i+1)/10) * 0.437 ) * 5
			value += sin( (y+time) * frequency * 3.31122 - (time + float(y+i+1)/10) * 4.269 ) * 2.5
			
			value *= amplitude 
			value -= (amplitude * 24)
			next_point.y += value
			points[0]  = Vector2( center.x + i * points_distance, center.y )
			points[1]  = prev_point
			points[2]  = next_point
			points[3]  = Vector2( center.x + ( i + 1  ) * points_distance, center.y )
			
			draw_polygon( points, colors )
			draw_line( prev_point, next_point, Color( 1, 1, 1 ), 1 )
			prev_point = next_point

func _draw():
	var center = Vector2(0, 200)
	var color = Color(0.7, 0.0, 1, 0.4)
	draw_wave_arc_poly(center, color)
